...
elseif($act == "all_read"){
    $result000 = mysqli_query($db,"SELECT * FROM user JOIN location ON user.user_location_id=location.location_id");
    $num=0;
    if($result000){
        while ($row000 = mysqli_fetch_array($result000)) { 
            $hello = $row000['location_id'];
            ...
            $result = mysqli_query($db,"SELECT * FROM zizi WHERE zizi_location_id = '$hello'" );
            if($result){
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) { 
                        $cow=$cow+$row["cow"];
                       ...
                       $data[$num] = array('cow' => $cow,'pig' => $pig, 'sheep' => $sheep, 'goat' => $goat,'location' => $row000["ward_name"]);
                       $num++;
                } else {
                    $data['animaldata'] = array('cow' => 0,'pig' => 0, 'sheep' => 0, 'goat' => 0);
                    echo json_encode($data);
    ...
    $data['sum'] = array('sum' => $num);
    echo json_encode($data);
}

elseif($act == "admin_read"){
    ...
    $result = mysqli_query($db,"SELECT * FROM zizi" );
    if($result){
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) { 
                $cow=$cow+$row["cow"];
